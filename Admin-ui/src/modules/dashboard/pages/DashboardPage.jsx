...
const RECENT_ACTIVITY = [
  { icon: CheckCircle2, color: 'text-emerald-400', title: 'Payment received', desc: '$850.00 from John Doe', time: '2m ago' },
  { icon: UserPlusIcon, color: 'text-blue-400', title: 'New user registered', desc: 'Alex Johnson joined', time: '15m ago' },
  { icon: Wrench, color: 'text-amber-400', title: 'Bike status updated', desc: 'Yamaha R15 set to Maintenance', time: '32m ago' },
  { icon: Calendar, color: 'text-violet-400', title: 'New booking created', desc: 'Booking #BK-1245 created', time: '1h ago' },
  { icon: XCircle, color: 'text-red-400', title: 'Payment failed', desc: 'Payment failed for booking #BK-1242', time: '2h ago' },
];
            ...
            <ul className="space-y-4">
              {RECENT_ACTIVITY.map((item, i) => (
                <li key={i} className="flex items-start gap-3">
                  <div className={cn('mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[var(--color-primary-soft)]', item.color)}>
        ...
        <CardContent>
          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3">
            {QUICK_LINKS.map((link) => (
              ...
              >
                <link.icon size={22} className={cn(link.color, 'group-hover:scale-110 transition-transform')} strokeWidth={1.75} />
