...
const emptyValue = '\u2014'
const formatDateTime = (value) => value ? new Date(value).toLocaleString([], { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' }) : emptyValue
const money = (value) => value === undefined || value === null ? emptyValue : `\u20B9${Number(value).toLocaleString('en-IN', { maximumFractionDigits: 2 })}`
...
function PriceRow({ label, value, emphasized = false, valueClassName = 'text-[#101c45]' }) {
    return <div className={`flex justify-between gap-4 text-[#40537e] ${emphasized ? 'border-t border-slate-100 pt-3' : ''}`}><span>{label}</span><strong className={valueClassName}>{value}</strong></div>
}
...
export default function BookingSummary({ vehicle, pickupAt, returnAt, availability, status = 'AVAILABLE', className = '' }) {
    ...
    const gstLabel = availability?.gstPercentage !== undefined ? `GST (${availability.gstPercentage}%)` : 'GST'
    const helmetAmount = availability ? availability.helmetAmount ?? 0 : undefined
    const totalAmount = availability?.totalAmountWithHelmet ?? availability?.totalAmount
    ...
    return (
        <aside className={`h-fit rounded-xl border border-slate-200 bg-white p-6 shadow-[0_8px_20px_rgba(28,55,113,0.045)] xl:sticky xl:top-24 ${className}`}>
            ...
            <section className="mt-7 border-t border-slate-200 pt-5"><p className="text-[13px] font-bold uppercase text-rideon-blue">Ride details</p><div className="mt-5 space-y-5"><DetailRow icon={CalendarDays} label="Pickup" value={formatDateTime(pickupAt)} /><DetailRow icon={CalendarDays} label="Return" value={formatDateTime(returnAt)} /><DetailRow icon={Clock3} label="Duration" value={availability?.durationHours ? `${availability.durationHours} Hours` : emptyValue} /></div></section>

            <section className="mt-8"><p className="text-[13px] font-bold uppercase text-rideon-blue">Pricing breakdown</p><div className="mt-4 space-y-4 text-[14px]"><PriceRow label={`Base amount${availability?.pricing?.packageName ? ` (${availability.pricing.packageName})` : ''}`} value={money(availability?.baseAmount)} /><PriceRow label="Helmet" value={money(helmetAmount)} /><PriceRow label="Platform fee" value={money(availability?.platformFee)} /><PriceRow label="Subtotal" value={money(availability?.subtotal)} emphasized /><PriceRow label={gstLabel} value={money(availability?.gstAmount)} /><PriceRow label="Security deposit (Refundable)" value={money(availability?.depositAmount)} /><PriceRow label="Included KM" value={includedKm === undefined ? emptyValue : `${includedKm} km`} valueClassName="text-[#148a36]" /><PriceRow label="Extra KM rate" value={availability?.extraKmRate === undefined ? emptyValue : `${money(availability.extraKmRate)} / km`} /></div></section>

            <section className="mt-7 overflow-hidden rounded-lg border border-[#c9dbff]"><div className="flex items-center justify-between bg-[#f7faff] px-5 py-4"><h3 className="text-[15px] font-bold">TOTAL AMOUNT</h3><strong className="text-[26px] leading-none text-rideon-blue">{money(totalAmount)}</strong></div><div className="space-y-4 border-t border-[#dbe6fa] px-5 py-4 text-[14px]"><div className="flex justify-between"><span className="text-[#40537e]">Includes</span><strong className="flex items-center gap-2 text-[#148a36]"><Check className="size-4" />{includedKm === undefined ? emptyValue : `${includedKm} km`}</strong></div><div className="flex justify-between"><span className="text-[#40537e]">Extra KM charge <Info className="ml-1 inline size-4" /></span><strong>After {includedKm === undefined ? emptyValue : `${includedKm} km`}</strong></div><div className="flex justify-between"><span className="text-[#40537e]">Payment status</span><strong className="rounded-md bg-[#fff2d8] px-2 py-1 text-xs text-[#c97800]">PENDING</strong></div></div></section>
