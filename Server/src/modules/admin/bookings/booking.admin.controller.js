...
export const collectAdditionalPaymentController = asyncHandler(async (req, res) => {
    const { id } = req.params
    const payment = await collectAdditionalPayment(id, req.body)
    await audit(req, 'CREATE', payment.id, { action: 'collect_additional_payment', bookingId: id, amount: payment.amount })
    res.status(201).json(new ApiResponse(201, 'Additional payment collected', payment))
})
